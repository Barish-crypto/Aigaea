...
class AiGaea:
    ...
    def welcome(self):
        print(
            f"""
        {Fore.GREEN + Style.BRIGHT}AI GAEA - BOT |  {Fore.YELLOW + Style.BRIGHT}Tool được phát triển bởi nhóm tele Airdrop Hunter Siêu Tốc (https://t.me/airdrophuntersieutoc)
            """
        )
    ...
    async def load_auto_proxies(self):
        ...
        try:
            async with ClientSession(timeout=ClientTimeout(total=6000)) as session:
    ...
    async def user_data(self, token: str, proxy=None, retries=500):
    ...
    async def user_earning(self, token: str, proxy=None, retries=50):
